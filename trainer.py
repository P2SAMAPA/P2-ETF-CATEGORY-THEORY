...
import config
import data_manager
from category_engine import yoneda_embedding
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (Category Theory) ===")
        ...
        for win in config.WINDOWS:
            ...
            scores = yoneda_embedding(returns, win, threshold=config.CORR_THRESHOLD)
            if scores is None:
                continue
        ...
        top_etfs = [{"ticker": ticker, "yoneda_score": float(score), "best_window": win} for ticker, (score, win) in sorted_etfs[:config.TOP_N]]

        print(f"  Top 3 ETFs by Yoneda score (weighted degree): {[e['ticker'] for e in top_etfs]}")
    ...
    local_path = Path(f"results/category_theory_{today}.json")
    with open(local_path, "w") as f:
        json.dump({"run_date": today, "universes": all_results}, f, indent=2)

    import push_results
    push_results.push_daily_result(local_path)
    print("\n=== Category Theory Portfolio Engine complete ===")
